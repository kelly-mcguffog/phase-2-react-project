...
const GenreContainer = ({ search, genre, allContent, handleEditMedia, setSearch }) => {
    const [beltPosition, setBeltPosition] = useState(0);

    const contentByGenre = allContent.filter(
        (content) => content.genre === genre
    );

    const matchingTitles = contentByGenre.filter((content) =>
    ...
    const renderContent = matchingTitles.map((content) => (
        <ContentCard
            key={content.id}
            content={content}
            // setSearch={setSearch}
