...
function HuluDramaContainer({displayHuluDramas, handleEditMedia, setSearch}){
    const [huluDramaBeltPosition, setHuluDramaBeltPosition] = useState(0)
    const renderHuluDramas = displayHuluDramas.map(content => <ContentCard key={content.id} content={content} setSearch={setSearch} handleEditMedia={handleEditMedia}/>)
